...
| `grafanaUI.plugins`                                     | Plugins installed via `GF_PLUGINS_PREINSTALL_SYNC` at startup (Grafana 12.x+). Format: `plugin-id@version`. | `grafana-lokiexplore-app@1.0.35,grafana-exploretraces-app@1.3.2,yesoreyeram-infinity-datasource@3.6.0,volkovlabs-form-panel@6.3.1` |
| `grafanaUI.extraPlugins`                                | Additional plugins to install alongside the default list. Same format as `plugins`. | `""` (none) |
| `grafanaUI.syntheticMonitoring.enabled`                 | Enable synthetic monitoring for Grafana Cloud    | `false`                               |
| `grafanaUI.syntheticMonitoring.plugins`                 | Synthetic monitoring plugin to install when enabled | `grafana-synthetic-monitoring-app@1.45.0` |
